...
const SignUpScreen = ({ navigation }) => {
  ...
  const validateCheck = () => {
    if (!termCheck) {
      setCheckError('You Must Accept Terms and conditions');
      return true;
    }
    setCheckError('');
    return false;
  ...
  const onSignup = () => {
    if (validateEmail() || validatePassword() || validateCheck()) {
      return;
    }
    dispatch(loginActions.requestSignup(email, password));
  };
